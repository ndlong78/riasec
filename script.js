...
const questions = [
    {
        id: 1,
        code: "R",
        text: "Em thích sửa chữa, lắp ráp các đồ vật, máy móc trong nhà."
    },
    {
        id: 2,
        code: "R",
...
function buildQuestions() {
    ...
    // Event delegation cho chọn đáp án
    questionsContainer.addEventListener("click", (e) => {
...
function renderResultSuggestions(top3, scores) {
    if (!top3 || !top3.length) {
...
async function renderResultUI(result) {
    ...
    renderResultSuggestions(top3, scores);
...
function refreshAdminTable(filters = {}) {
    ...
    if (rendered === 0) {
        const tr = document.createElement("tr");
        tr.innerHTML = `<td colspan="6" class="admin-empty-row">Không có kết quả phù hợp.</td>`;
        adminResultsTable.appendChild(tr);
    }
}
